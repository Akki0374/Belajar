...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Handle multiple Site IDs (comma-separated) or single Site ID
    let siteIdCondition: ReturnType<typeof sql> | string;

    if (siteId && siteId !== "---") {
      const siteIds = siteId.split(",").map((id) => id.trim().toUpperCase());

      if (siteIds.length === 1) {
        // Single Site ID - use LIKE for partial matching
        siteIdCondition = sql`AND t1."BTS Name" LIKE ${`%${siteIds[0]}%`}`;
      } else {
        // Multiple Site IDs - create OR conditions with LIKE for each Site ID
        const likeConditions = siteIds.map(
          (siteId) => sql`t1."BTS Name" LIKE ${`%${siteId}%`}`
        );
        siteIdCondition = sql`AND (${sql.join(likeConditions, sql` OR `)})`;
      }
    } else {
      // No Site ID provided
      siteIdCondition = sql``;
